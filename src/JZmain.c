/* ------------------------------------------------------ XCutXCodeXSkip
     This file (JZmain.c) is part of QTL Cartographer
...
/*  Main function for JZmapqtl, experimental code.   Not yet active.*/
#include "Main.h"
...
{
  char *chptr,*purpose;
  int nopts,ii,automatic, rows, t ,marks,**srranks,srm;
  params *theparams;
  linpakws *lnpk;
#ifndef WIN32
 ...
 /*  Simulate the UNIX shell with ccommand, this should also
  ...
  if ( theparams->Model == 7 || theparams->ihypo > 39 )   /*  Model 7 requires the output of Eqtl, as do hypothesis tests > 39.   */
...
/*  Model 7 uses an eqtl output file to determine its cofactors.   Ihyp > 39 uses them for hypothesis testing. */
...
/* Get the results from a previous run of SRmapqtl */
...
/* Disable permutations and bootstraps in this module.   */
  theparams->perms = theparams->boots = 0; 
/*   Do the work.   */
  if ( theparams->Model == 9 )     
    prep_multiregress(theparams,theparams->themap,theparams->thedata,theparams->thegenome,lnpk,argv[0],chptr );
  else if ( theparams->ihypo == 40 )    /*  Same genotypes in different environs   */
    do_gbye_same(theparams,theparams->themap,theparams->theqtls,theparams->thedata,theparams->thegenome,lnpk,argv[0],chptr,srranks);
  else if ( theparams->ihypo == 50 )    /*  Random genotypes in different environs */
    do_gbye_diff(theparams,theparams->themap,theparams->theqtls,theparams->thedata,theparams->thegenome,lnpk,argv[0],chptr,srranks);
  else if ( theparams->ihypo == 60 )    
    do_pleiotropy(theparams,lnpk,argv[0],chptr,srranks);
  else                                 /*  Scan the genome   ihypo = 1, 2, 3, 10, 14, 20, 30, 31, 32, 34   */
    do_scan(theparams,theparams->themap,theparams->theqtls,theparams->thedata,theparams->thegenome,lnpk,argv[0],chptr,srranks);
/* Clean up...*/
  theparams->srm = srm;
  theparams->walk = theparams->walk * (FPN) 100.0;
  theparams->Inter = 0;
...
void update_opts(char **opt,char  **opt_v,char  **opt_e, int nopts, params *theparams, int flag)
{
  ...
  if (flag == 1) {
    ...
    strcpy(opt_e[5], "SRmapqtl results (Model 6,8)");
    strcpy(opt_e[6], "Eqtl results (Model 7)");
    strcpy(opt_e[7], "Random Number Seed");
    strcpy(opt_e[8], "Model [3,6,7,8,9]");
    strcpy(opt_e[9], "Trait to analyze");
    strcpy(opt_e[10], "Chromosome to analyze (0=>all)");
    strcpy(opt_e[11], "Walking speed in cM");
    strcpy(opt_e[12], "Number of Background Parameters (Models 6,7,8)");
    strcpy(opt_e[13], "Window Size in cM (Models 6,7,8)");
    strcpy(opt_e[14], "Hypothesis test [10,30,40,50,60]");
  }
