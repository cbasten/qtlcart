...
$ZMAPQTL = $opt_f;
#
#   If a ZipermC.out file is given, then process it.
#
if ( length($opt_f) > 0 ) {   
    $indata = 0;
	open ZMAPQTL  or die "Can't open ZipermC.out results file $ZMAPQTL $!\n";
    while ( <ZMAPQTL> ) {
      chomp;
...
while (<>) {
      ...
      if ( $indata == 0 ) {  #  we are in the header, so get relevant information
        ...
        if ( $opt_f eq "" ) {
          $pvalue = 0.0;
          $count = 0;
          print "\n  $row     $chromosome    $marker    $position   $rest[$opt_C-3]    $pvalue        $count";
        }
        else {
          $key =  $chromosome . ":" . $marker . ":" . $position ;   
          $counts{$key}++ if ( $rest[$opt_C-3] > $lrsample{$key} ) ;
          $pvalue = $counts{$key} / $permutation;
          print "\n  $row     $chromosome    $marker    $position   $lrsample{$key}      $pvalue        $counts{$key}";
        }
      }
...
#   
$header = "\#      123456789   -filetype ZipermC.out
\#
\#       QTL Cartographer  
...
\#Row Chrom Mark Position  LR\(Samp\)    P-Val     Count      -perm $permutation   -start";
...
B<CWTupdate.pl> reads from the standard input and writes to the standard
output. It can do one of two things.   The first is to initialize a file
of likelihood ratios in the F<ZipermC.out> format from a F<Zmapqtl.out>
file. The second is to read the results of a run of B<Zmapqtl> and the
current F<ZipermC.out> file and update the results.   It is a B<Perl>
...
specify the F<ZipermC.out> file for processing.   If it is not given, then the 
